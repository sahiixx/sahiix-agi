...
class MissionRunner:
    ...
    async def create_mission(self, goal: str) -> Mission:
        ...
        try:
            response = await asyncio.wait_for(
                self.director.chat(
                    'Break down this goal into 3-5 concrete steps. Each step should specify which agent to use (director, coder, researcher, sysadmin, architect) and what to do.\n\nGoal: ' + goal + '\n\nFormat each step as:\nSTEP <number>: <agent_name> | <description>\n\nKeep descriptions under 100 words.',
                    agent_name="director"
                ),
                timeout=30.0
            )
    ...
    async def _execute_step(self, step: MissionStep):
        ...
        tool_registry = getattr(self.director, 'tools', None)
        if tool_registry and step.tool and hasattr(tool_registry, 'tools') and step.tool in tool_registry.tools:
